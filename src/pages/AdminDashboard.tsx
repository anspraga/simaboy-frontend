                          ...
                          {Array.from({ length: Math.min(totalPages, 5) }, (_, i) => {
                            ...
                            return (
                              <button key={page} onClick={() => setCurrentPage(page)}
                                className={`px-3 py-1.5 rounded-lg text-xs font-bold border transition ${page === currentPage ? 'bg-indigo-500 border-indigo-500 text-white shadow-sm' : 'border-slate-200 text-slate-500 hover:bg-white hover:shadow-sm'
                                  }`}>{page}</button>
                            );
                          })}
